...
async function resolveSourceUrl(): Promise<{ url: string; sku?: string; productId?: string }> {
...
async function main(): Promise<void> {
  ...
  const { url: blobUrl } = await resolveSourceUrl();
  ...
  const payload = {
    product: {
      ...
      media: wixMediaUrl ? { mediaItems: [{ url: wixMediaUrl, title: fileName }] } : undefined,
    },
    returnEntity: true,
  };
  ...
  // 3) Read-back con productsV3.getProduct
  ...
  if (productId) {
    ...
    mediaAccepted = Boolean(media && (media.mediaItems?.length > 0 || media.items?.length > 0));
  }
